...
class Waypointer(object):
    ...
    def generate_final_trajectory(self, coarse_trajectory):
        # TODO: the use of bezier is a hack. a proper planning should be used on the case.
    ...
    """
    # TODO: probably this does not belong on this class
    def convert_points_to_world(self, map_points):

        world_points = []
        for point in map_points:
            # print point
            # print self._converter.convert_to_pixel(point)
            world_points.append(self._converter.convert_to_world(np.array(point)))

        return world_points

    def smooth_trajectory(self, coarse_trajectory):
        total_course_trajectory_distance = 0
        previous_point = coarse_trajectory[0]
        for i in range(1, len(coarse_trajectory)):
            total_course_trajectory_distance += sldist(coarse_trajectory[i], previous_point)
        ...
        for point in points:
            smoothed_points.append(point.tolist())
    ...
    def graph_to_waypoints(self, next_route, smoothed=False):

        """

        Args
            next_route:
            smoothed: if you are going to smooth the waypoints, by that we mean generating
                     sub waypoints and smoothing the curves with a bezier curve. That is suboptimal


        :return:
            a list of world points and a list of
        """
        ...
        if smoothed:
            smoothed_shifted_lane_vec = self.smooth_trajectory(shifted_lane_vec)
            return self.convert_points_to_world(smoothed_shifted_lane_vec), smoothed_shifted_lane_vec
        else:
            return self.convert_points_to_world(shifted_lane_vec), shifted_lane_vec
