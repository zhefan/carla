...
# Refactoring of a code from Matthias Muller
...
class Autopilot(Agent):
    ...
    def controller(self, wp_angle, wp_mag, wp_angle_speed):
        ...
        if (self.usePID):
            ...
            if (self.current_speed > self.target_speed):
                throttle_adjusted = (1 / (
                        self.current_speed + 2 - self.target_speed) * self.throttle_max + 2 * self.throttle_max / 3) * self.speed_factor
    ...
    def run_step(self, measurements, sensor_data, target):
        ...
        wp_vector_speed, wp_mag_speed = self.get_vec_dist(self.wp_speed[0], self.wp_speed[1], loc_x_player,
                                                          loc_y_player)
