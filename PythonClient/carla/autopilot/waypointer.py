...
class Waypointer(object):
    ...
    def __init__(self, config, debug=False):
        ...
        self._converter = Converter(config.city_file, 16.43, 50.0)
    ...
    def get_next_waypoints(self, source, source_ori, target, target_ori):
        ...
        if self.debug:
            self._write_point_on_map(self._converter.convert_to_pixel(source), [255, 0, 255, 255], size=7)

            self._write_point_on_map(self._converter.convert_to_pixel(target), [255, 255, 255, 255], size=6)
        ...
        if distance_node > 2 and self._previous_source != track_source:
            ...
            self.points = self.graph_to_waypoints(self._route[1:(1 + self.way_key_points_predicted)],
            ...
            if distance(self.previous_map, self._converter.convert_to_pixel(source)) > 3.0:
                ...
                for point in self.last_map_points:
                    point_vec = self._get_unit(np.array(self._converter.convert_to_pixel(source)), point)
                    ...
                    if (cross_product > 0.0 and sldist(point, self._converter.convert_to_pixel(source)) < 50) or sldist(
                            point,
                        ...
                        self.last_trajectory.remove(
                            self._converter.convert_to_world(point))  # = [self.make_world_map(point)] + self.last_trajc
